...
def test_remove_deletes_the_expense(expenses_list: list[dict[str, Any]]) -> None:
    expense_id_to_remove = 2
    remove(expenses_list, expense_id=expense_id_to_remove)

    assert 2 not in [e["id"] for e in expenses_list]

def test_remove_returns_the_removed_expense(expenses_list: list[dict[str, Any]]) -> None:
    expense_id_to_remove = 2
    removed_expense = remove(expenses_list, expense_id=expense_id_to_remove)

    assert removed_expense["id"] == expense_id_to_remove

def test_remove_leaves_other_expenses_untouched(expenses_list: list[dict[str, Any]]) -> None:
...
def test_remove_raises_for_an_unknown_id(expenses_list: list[dict[str, Any]]) -> None:
