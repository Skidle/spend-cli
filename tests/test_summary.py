...
from spend.commands import Summary, summary
...
def expenses_list() -> list[Expense]:
    return [
        Expense(id=1, amount=30, category="food", date="2026-08-20"),
        Expense(id=3, amount=60, category="transport", date="2026-08-20"),
        Expense(id=2, amount=10, category="food", date="2026-08-15"),
    ]

def test_summary_totals_amounts_per_category(expenses_list: list[Expense]) -> None:
    result = summary(expenses_list)

    assert result.rows[0].total == 60
    assert result.rows[1].total == 40

def test_summary_rows_are_sorted_by_total_descending(expenses_list: list[Expense]) -> None:
    result = summary(expenses_list)

    assert result.rows[0].category == "transport"
    assert result.rows[1].category == "food"

def test_summary_calculates_fractions(expenses_list: list[Expense]) -> None:
    result = summary(expenses_list)

    assert result.rows[0].fraction == 0.6
    assert result.rows[1].fraction == 0.4

def test_summary_filters_by_since_before_totalling(expenses_list: list[Expense]) -> None:
...
def test_summary_does_not_mutate_the_input(expenses_list: list[Expense]) -> None:
