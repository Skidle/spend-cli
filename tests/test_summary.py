...
def test_summary_totals_amounts_per_category(expenses_list):
    result = summary(expenses_list)

    assert result["rows"][0]["total"] == 60
    assert result["rows"][1]["total"] == 40

def test_summary_rows_are_sorted_by_total_descending(expenses_list):
    result = summary(expenses_list)

    assert result["rows"][0]["category"] == "transport"
    assert result["rows"][1]["category"] == "food"

def test_summary_calculates_fractions(expenses_list):
    result = summary(expenses_list)

    assert result["rows"][0]["fraction"] == 0.6
    assert result["rows"][1]["fraction"] == 0.4

def test_summary_filters_by_since_before_totalling(expenses_list):
...
def test_summary_returns_empty_rows_and_zero_total_for_no_expenses():
    expected = {
        "rows": [],
        "grand_total": 0,
    }
...
def test_summary_does_not_mutate_the_input(expenses_list):
